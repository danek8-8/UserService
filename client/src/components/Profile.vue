...
export default {
  ...
  methods: {
    async userProfile() {
      try {
        const authToken = localStorage.getItem('authToken'); // Получаем токен из localStorage (или другого хранилища)
        const adminToken = localStorage.getItem('adminToken'); // Получаем токен из localStorage (или другого хранилища)
        ...
        /*await axios.get(`http://localhost:5000/api/users`, {
          headers: { Authorization: `Bearer ${adminToken}` } // Передаем токен в заголовке
        });*/
